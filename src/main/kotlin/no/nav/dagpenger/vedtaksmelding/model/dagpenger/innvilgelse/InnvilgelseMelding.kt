...
import no.nav.dagpenger.vedtaksmelding.model.dagpenger.innvilgelse.InnvilgelseBrevblokker.INNVILGELSE_DAGPENGEPERIODE_HVIS_TOM_DATO_DEL_3
...
class InnvilgelseMelding(
    ...
    override val brevBlokkIder: List<String>
        get() {
            val fasteBrevblokkerInnvilgelse =
                listOf(
    ...
    private fun sanksjonBlokker(): List<String> =
        if (erIlagtSanksjonsperiodeVedSelvforskyldtArbeidsløshet()) {
            listOf(INNVILGELSE_SANKSJON_DEL_1.brevblokkId, INNVILGELSE_SANKSJON_DEL_2.brevblokkId)
        } else {
            emptyList()
        }
